...
function gmailWalkAttachments(part,out){
  ...
  if(aid||(fn&&!/^multipart\//i.test(mt))){
    out.push({filename:fn||"unnamed",mimeType:mt||"application/octet-stream",size:(part.body&&part.body.size)||0,attachmentId:aid||null,idLen:aid?String(aid).length:0,hasInline:!!(part.body&&part.body.data)});
  }
...
// Filename lookup so agents never have to round-trip Gmail attachmentIds
// (those IDs routinely exceed MCP/card payload limits and arrive truncated).
function gmailFindPartByFilename(part,filename){
  if(!part||!filename)return null;
  if(part.filename===filename&&part.body&&(part.body.attachmentId||part.body.data))return part;
  if(part.parts)for(const p of part.parts){const f=gmailFindPartByFilename(p,filename);if(f)return f;}
  return null;
}
...
async function gmailGetAttachment(env,email,messageId,attachmentId,filename){
  ...
  let part=attachmentId?gmailFindPart(m.payload,attachmentId):null;
  if(!part&&filename)part=gmailFindPartByFilename(m.payload,filename);
  if(!part)throw new Error("attachment not found on that message");
  const aid=part.body&&part.body.attachmentId;
  if(!aid&&part.body&&part.body.data){
    const bytes=b64urlToBytes(part.body.data);
    return {id:m.id||messageId,filename:part.filename||"unnamed",mimeType:part.mimeType||"application/octet-stream",size:(part.body.size)||bytes.length,bytes};
  }
  if(!aid)throw new Error("attachment not found on that message");
...
async function callTool(env,name,args){
  ...
  if(name==="get_attachment"){
    ...
    if(!uid||(!aid&&!fn))throw new Error("uid and filename or attachmentId required");
    ...
    const att=await gmailGetAttachment(env,a,uid,aid,fn);
